...
class Player2(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.moving:
            ...
            self.x += math.sin(math.radians(self.angle + 180))
            self.y += math.cos(math.radians(self.angle + 180))
            self.rect.x = self.x
            self.rect.y = self.y
        if pygame.sprite.spritecollideany(self, borders, False) \
                or pygame.sprite.spritecollideany(self, player1_group, False):
            self.moving = False
            self.x -= math.cos(math.radians(self.angle)) + 180
            self.y -= math.sin(math.radians(self.angle)) + 180
            self.rect.x = self.x
            self.rect.y = self.y
